...
Starting from v1.2, Clair3 natively supports GPU acceleration on NVIDIA (CUDA) and AMD Instinct (ROCm) hardware. Using a single GPU, Clair3 can complete an ONT WGS 30x whole-genome variant calling in ~20 minutes on a Linux server with 32 CPU threads and an NVIDIA GeForce RTX 4090.

The quickest way to run Clair3 on NVIDIA GPU is the pre-built Docker / Singularity image `hkubal/clair3:v2.0.1_gpu` (built on CUDA 12.1, bundled with all pre-trained models). For AMD Instinct GPUs, build the ROCm image from `Dockerfile.gpu` (see [Option 4](#option-4-amd-instinct-rocm-on-linux) below); the same pre-trained models apply.
...
- amdgpu kernel driver + ROCm runtime on the host (ROCm 7.0 or 7.1). No container toolkit is needed; GPUs are passed through at `docker run` with `--device=/dev/kfd --device=/dev/dri --group-add=video`.
- The PyTorch ROCm wheel index minor must match the host/image ROCm minor.
...
# AMD Instinct (ROCm image built from Dockerfile.gpu)
docker run --rm --device=/dev/kfd --device=/dev/dri --group-add=video \
 clair3:rocm rocm-smi
```
...
# NVIDIA: pick the right CUDA version for your driver from https://pytorch.org/get-started/locally/
uv pip install torch torchvision --index-url https://download.pytorch.org/whl/cu121
# AMD Instinct (ROCm): pick the rocmX.Y index matching your host ROCm (7.0 or 7.1)
...
## Option 4. AMD Instinct (ROCm on Linux)

Clair3's GPU path runs unchanged on AMD Instinct GPUs through ROCm PyTorch: `--device=cuda:0`, `torch.cuda.*`, and `CUDA_VISIBLE_DEVICES` all map onto HIP, so no Clair3 options change.

Build the ROCm GPU image from the repository (the default base is `rocm/dev-ubuntu-22.04:7.1-complete`; change the base tag and the matching `whl/rocm7.0|7.1` index in `Dockerfile.gpu` to target a different ROCm minor):

```bash
docker build -f Dockerfile.gpu -t clair3:rocm .
```

Run with ROCm device passthrough (not `--gpus all`):
...
- Select specific GPUs with `--device=cuda:0,1` (Clair3) or by exporting `CUDA_VISIBLE_DEVICES` / `HIP_VISIBLE_DEVICES` before the run.
- For a host (no-Docker) install, follow the step-by-step Conda flow above and install the ROCm PyTorch wheel (`--index-url https://download.pytorch.org/whl/rocm7.1`) instead of the CUDA wheel.
